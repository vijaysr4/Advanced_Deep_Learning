...
class DDPMSampler(nn.Module):
    ...
    def sample(
        ...
        for step, (g_cur, g_next) in enumerate(zip(gammas[:-1], gammas[1:])):
            ...
            x_mean = ( torch.sqrt(alpha_cur) * (1 - g_next) * x_cur +
                       torch.sqrt(g_next) * (1 - alpha_cur) * x0 ) / (1 - g_cur)
            var_t = (1 - alpha_cur) * (1 - g_next) / (1 - g_cur)
            noise = torch.randn_like(x_cur)
            x_next = x_mean + torch.sqrt(var_t) * noise
            # ------------------------------------------------------------------------- #
